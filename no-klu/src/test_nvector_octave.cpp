...
int main(int argc, char *argv[])
{
  ...
  realtype *v_data = (realtype *) malloc(length * sizeof(realtype));;
  Test_Init(NULL);
  ...
  U = N_VNew_Octave(length, sunctx);
  retval = N_VEnableFusedOps_Octave(U, SUNFALSE);
  if (U == NULL || retval != 0) {
    N_VDestroy(W);
    N_VDestroy(X);
    N_VDestroy(Y);
  ...
  V = N_VNew_Octave(length, sunctx);
  retval = N_VEnableFusedOps_Octave(V, SUNTRUE);
  if (V == NULL || retval != 0) {
    N_VDestroy(W);
    N_VDestroy(X);
    N_VDestroy(Y);
