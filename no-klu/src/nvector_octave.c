...
N_Vector N_VNewEmpty_Octave(sunindextype length)
{
  N_Vector v;
  N_VectorContent_Octave content;
  ...
  v->ops->nvwl2norm      = N_VWL2Norm_Octave;
  v->ops->nvl1norm       = N_VL1Norm_Octave;
  ...
  v->ops->nvdotprodlocal     = N_VDotProd_Octave;
  v->ops->nvmaxnormlocal     = N_VMaxNorm_Octave;
  v->ops->nvminlocal         = N_VMin_Octave;
  v->ops->nvl1normlocal      = N_VL1Norm_Octave;
  v->ops->nvinvtestlocal     = N_VInvTest_Octave;
  v->ops->nvconstrmasklocal  = N_VConstrMask_Octave;
  v->ops->nvminquotientlocal = N_VMinQuotient_Octave;
  v->ops->nvwsqrsumlocal     = N_VWSqrSumLocal_Octave;
  v->ops->nvwsqrsummasklocal = N_VWSqrSumMaskLocal_Octave;

  /* Create content */
  content = NULL;
  content = (N_VectorContent_Octave) malloc(sizeof *content);
...
N_Vector N_VNew_Octave(sunindextype length)
{
  N_Vector v;
  realtype *data;

  v = NULL;
  v = N_VNewEmpty_Octave(length);
  if (v == NULL) return(NULL);
  ...
  if (length > 0) {
    ...
    if(data == NULL) { N_VDestroy_Octave(v); return(NULL); }
...
N_Vector N_VMake_Octave(sunindextype length, realtype *v_data)
{
  N_Vector v;
...
void N_VPrintFile_Octave(N_Vector x, FILE* outfile)
{
  sunindextype i, N;
  realtype *xd;
...
{
  return(SUNRsqrt(N_VWSqrSumLocal_Octave(x, w)/(NV_LENGTH_S(x))));
}
...
{
  return(SUNRsqrt(N_VWSqrSumMaskLocal_Octave(x, w, id) / (NV_LENGTH_S(x))));
}
