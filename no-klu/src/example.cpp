 ...
 * -----------------------------------------------------------------
 * Example problem for IDA: 2D heat equation, Col, GMRES.
 *
...
int main()
{
  ...
  uu = N_VNew_Col(NEQ, ctx);
  if(check_retval((void *)uu, "N_VNew_Col", 0)) return(1);

  up = N_VClone(uu);
  if(check_retval((void *)up, "N_VNew_Col", 0)) return(1);

  res = N_VClone(uu);
  if(check_retval((void *)res, "N_VNew_Col", 0)) return(1);

  constraints = N_VClone(uu);
  if(check_retval((void *)constraints, "N_VNew_Col", 0)) return(1);
  ...
  data->pp = N_VClone(uu);
  if(check_retval((void *)data->pp, "N_VNew_Col", 0)) return(1);
  ...
  std::cout<<"Succesfully completed.";
  return(0);
}
...
static void PrintHeader(realtype rtol, realtype atol)
{
  printf("\nidaHeat2D_kry: Heat equation, Col example problem for IDA \n");
