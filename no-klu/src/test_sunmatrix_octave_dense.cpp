...
int main(int argc, char *argv[])
{
  ...
  A = OCTDenseMatrix(matrows, matcols, sunctx);
  I = NULL;
  if (square)
    I = OCTDenseMatrix(matrows, matcols, sunctx);

  /* Fill matrices and vectors */
  Adata = OCTDenseMatrix_Data(A);
  for(j=0; j < matcols; j++) {
    for(i=0; i < matrows; i++) {
...
//   }
  ...
  if (fails) {
    ...
    printf("\nA =\n");
  //   OCTDenseMatrix_Print(A,stdout);
  //   if (square) {
  //     printf("\nI =\n");
  //     OCTDenseMatrix_Print(I,stdout);
  //   }
  //   printf("\nx =\n");
  //   N_VPrint_Serial(x);
  //   printf("\ny =\n");
...
int check_matrix(SUNMatrix A, SUNMatrix B, realtype tol)
{
  ...
  /* get data pointers */
  Adata = OCTDenseMatrix_Data(A);
  Bdata = OCTDenseMatrix_Data(B);

  /* get and check data lengths */
  Aldata = OCTDenseMatrix_LData(A);
  Bldata = OCTDenseMatrix_LData(B);
...
{
  realtype *Adata = OCTDenseMatrix_Data(A);
  if (Adata == NULL)
    return SUNFALSE;
  else
    return SUNTRUE;
...
{
  if (OCTDenseMatrix_Rows(A) == OCTDenseMatrix_Columns(A))
    return SUNTRUE;
  else
    return SUNFALSE;
}
