...
#include "sunlinsol_klu.h"           /* access to KLU linear solver          */
...
int main(void)
{
  ...
  if (check_retval((void *)uu, "N_VNew_Octave", 0))
    return (1);
  up = N_VClone(uu);
  if (check_retval((void *)up, "N_VNew_Octave", 0))
    return (1);
  res = N_VClone(uu);
  if (check_retval((void *)res, "N_VNew_Octave", 0))
    return (1);
  constraints = N_VClone(uu);
  if (check_retval((void *)constraints, "N_VNew_Octave", 0))
    return (1);
  id = N_VClone(uu);
  if (check_retval((void *)id, "N_VNew_Octave", 0))
    return (1);
             ...
             N_Vector tempv1, N_Vector tempv2, N_Vector tempv3)
{
  ...
  sunindextype *colptrs = OCTSparseMatrix_IndexPointers(JJ);
  sunindextype *rowvals = OCTSparseMatrix_IndexValues(JJ);
  realtype *data = OCTSparseMatrix_Data(JJ);
