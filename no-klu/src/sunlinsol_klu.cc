...
int SUNLinSolSetup_KLU(SUNLinearSolver S, SUNMatrix A)
{
  ...
  if (FIRSTFACTORIZE(S)) {
    ...
    if (SYMBOLIC(S))
      sun_klu_free_symbolic(&SYMBOLIC(S), &COMMON(S));
    SYMBOLIC(S) = sun_klu_analyze(OCTSparseMatrix_NP(A),
                                  (KLU_INDEXTYPE*) OCTSparseMatrix_IndexPointers(A),
                                  (KLU_INDEXTYPE*) OCTSparseMatrix_IndexValues(A),
                                  &COMMON(S));
    if (SYMBOLIC(S) == NULL) {
    ...
    if(NUMERIC(S))
      sun_klu_free_numeric(&NUMERIC(S), &COMMON(S));
    NUMERIC(S) = sun_klu_factor((KLU_INDEXTYPE*) OCTSparseMatrix_IndexPointers(A),
                                (KLU_INDEXTYPE*) OCTSparseMatrix_IndexValues(A),
                                OCTSparseMatrix_Data(A),
                                SYMBOLIC(S),
                                &COMMON(S));
    if (NUMERIC(S) == NULL) {
      LASTFLAG(S) = SUNLS_PACKAGE_FAIL_UNREC;
    ...
    retval = sun_klu_refactor((KLU_INDEXTYPE*) OCTSparseMatrix_IndexPointers(A),
                              (KLU_INDEXTYPE*) OCTSparseMatrix_IndexValues(A),
                              OCTSparseMatrix_Data(A),
                              SYMBOLIC(S),
                              NUMERIC(S),
    ...
    if ( COMMON(S).rcond < uround_twothirds ) {
	 ...
	 Compute more accurate estimate */
      retval = sun_klu_condest((KLU_INDEXTYPE*) OCTSparseMatrix_IndexPointers(A),
                               OCTSparseMatrix_Data(A),
                               SYMBOLIC(S),
                               NUMERIC(S),
                               &COMMON(S));
